...
const archiveEntries = Object.fromEntries(
  archivePaths.map((path) => [path, gitBytes(["show", `HEAD:skills/cycle/${path}`])]),
)
...
const records = archivePaths.map((path) => {
  ...
  if (Buffer.compare(Buffer.from(extracted), committed) !== 0) throw new Error(`local Skill extracted file differs: ${path}`)
...
await writeFile(join(output, `${artifactName}.provenance.json`), `${JSON.stringify({
  artifact: { name: artifactName, sha256: digest, size: bytes.length },
  buildType: "deterministic-fflate-zip",
