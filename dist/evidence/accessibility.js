...
export function parseSnapshot(raw) {
    ...
    let counted = 0;
    const parseNode = (value, depth) => {
        if (depth > MAX_DEPTH)
            throw new SnapshotRejected(`the accessibility tree nests beyond ${MAX_DEPTH} levels`);
        counted += 1;
        if (counted > MAX_NODES)
            throw new SnapshotRejected(`the accessibility tree exceeds ${MAX_NODES} nodes`);
        const node = exactKeys(value, ["children", "level", "name", "role"]);
        const level = node["level"];
        if (level !== null && (!Number.isInteger(level) || level < 1 || level > 12)) {
    ...
    return {
        ...
        nodes: root["nodes"].map((node) => parseNode(node, 1)),
        url,
    };
}
export function inspectAccessibility(snapshot) {
    const findings = [];
    const flat = [];
    const walk = (nodes) => {
    ...
    walk(snapshot.nodes);
    const at = (position, rule, severity, summary) => {
    ...
    flat.forEach((node, index) => {
        ...
        if (node.role !== "heading")
            return;
        if (!named)
            at(position, "a11y/empty-heading", "medium", "a heading has no text");
        const level = node.level ?? 0;
        if (previousHeading > 0 && level > previousHeading + 1) {
            at(position, "a11y/heading-order", "low", `heading level jumps from ${previousHeading} to ${level}, leaving a gap in the outline`);
        }
        if (level > 0)
...
function exactKeys(raw, keys) {
    ...
    if (actual.length !== expected.length || actual.some((key, index) => key !== expected[index])) {
        throw new SnapshotRejected(`a snapshot object must have exactly these keys: ${expected.join(", ")} (received: ${actual.join(", ") || "none"})`);
    }
    return raw;
}
...
function optionalText(raw, field) {
