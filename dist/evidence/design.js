...
export function isInterfaceFile(path) {
    return STYLE_FILE.test(path) || MARKUP_FILE.test(path);
}
export function inspectDesign(files) {
    const findings = [];
...
export function parseColour(value) {
    ...
    if (short !== null) {
        return [0, 1, 2].map((index) => Number.parseInt(short[index + 1].repeat(2), 16));
    }
    const long = /^#([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/u.exec(text);
    if (long !== null) {
        return [0, 1, 2].map((index) => Number.parseInt(long[index + 1], 16));
    }
...
function contrast(file) {
    ...
    for (const block of styleBlocks(file.content)) {
        ...
        const background = parseColour(declaration(block.body, "background-color") ?? "") ??
...
function focusNotVisible(file) {
    if (/:focus-visible/u.test(file.content))
        return [];
...
function positiveTabindex(file) {
    return scan(file, /tabindex\s*=\s*[{"']?\s*(\d+)/giu, (match) => Number(match[1]) > 0
        ? {
...
function interactiveWithoutKeyboard(file) {
    return scan(file, new RegExp(`<(${PASSIVE_ELEMENTS})\\b([^>]*)>`, "gisu"), (match) => {
        const attributes = match[2] ?? "";
        if (!/\bon(?:Click|click)\b/u.test(attributes))
            return null;
        if (/\bon(?:KeyDown|KeyUp|KeyPress|keydown|keyup|keypress)\b/u.test(attributes))
            return null;
        if (/\brole\s*=/u.test(attributes))
            return null;
        return {
...
function imageWithoutAlt(file) {
    return scan(file, /<img\b([^>]*)>/gisu, (match) => /\balt\s*=/u.test(match[1] ?? "")
        ? null
        : {
...
function reducedMotion(file) {
    if (/prefers-reduced-motion/u.test(file.content))
        return [];
...
function fixedWidth(file) {
    if (/@media/u.test(file.content))
        return [];
    return scan(file, /(?:^|[\s;{])width\s*:\s*(\d{3,})px/gimu, (match) => Number(match[1]) >= 600
        ? {
...
function invalidNesting(file) {
    ...
    while ((match = paragraph.exec(file.content)) !== null) {
        ...
        if (tag === undefined || !BLOCK_IN_PARAGRAPH.has(tag))
            continue;
        findings.push({
            file: file.path,
...
function missingErrorState(file) {
    if (!FETCHES.test(file.content) || HANDLES_FAILURE.test(file.content))
        return [];
...
function scan(file, pattern, decide) {
    const findings = [];
    let match;
    while ((match = pattern.exec(file.content)) !== null) {
