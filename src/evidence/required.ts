...
interface Requirement {
  ...
  /** What the change touched, which is why the proof is required. */
  readonly paths: RegExp
  readonly reason: string
  /** A project-native gate whose invocation matches this already supplies the proof. */
  readonly supplied: RegExp
}

const DEPENDENCY_MANIFEST =
  /(^|\/)(package\.json|package-lock\.json|pnpm-lock\.yaml|yarn\.lock|bun\.lockb?|Cargo\.toml|Cargo\.lock|go\.mod|go\.sum|pyproject\.toml|poetry\.lock|requirements[^/]*\.txt|Gemfile|Gemfile\.lock|composer\.json|composer\.lock|pom\.xml|build\.gradle[^/]*)$/iu

const INTERFACE_FILE = /\.(tsx|jsx|vue|svelte|css|html)$|(^|\/)(components|pages|frontend)(\/|$)/iu

// Segment bounded, so "authorship" and "consolidate" do not demand a security proof while
// "src/auth/session.ts" and "lib/rbac-policy.go" do.
const SECURITY_SURFACE =
  /(^|[/\-_.])(auth|authn|authz|login|signin|session|sessions|token|tokens|permission|permissions|role|roles|rbac|crypto|password|passwords|secret|secrets|credential|credentials|sanitize|sanitizer|sanitise|validation|validator|escape)([/\-_.]|$)/iu

/**
 * Section 7.3 of the specification. When a change touches a layer that requires proof and the
 * project has no gate that supplies it, the missing gate is inserted and fails by construction.
 * This is the mechanism: a layer with no proof is a failure, not a silence.
 */
const REQUIREMENTS: readonly Requirement[] = [
  ...
  },
  {
    ...
    reason:
      "the change touches authentication, authorization, input handling or secrets and no gate " +
...
/**
 * `advisory` exists for immature codebases: it records the missing proof as a warning instead of
 * failing the candidate. It never removes the gate, because the record is the point.
 */
export function requiredMissingGates(
  changed: readonly ChangedFile[],
  discovered: readonly Gate[],
  ...
  for (const requirement of REQUIREMENTS) {
    ...
    if (recorded.has(requirement.name)) continue
    if (invocations.some((invocation) => requirement.supplied.test(invocation))) continue
