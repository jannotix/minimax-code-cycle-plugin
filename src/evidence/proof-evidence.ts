...
import { PROOF_TIMEOUT_SECONDS, type ProofResult } from "./proof.ts"

const CLASS_PATTERN = /^[a-z][a-z0-9-]{2,47}$/u

export function proofGateName(vulnerabilityClass: string): string {
  const normalized = vulnerabilityClass.trim().toLowerCase().replaceAll(/[\s_]+/gu, "-")
  if (!CLASS_PATTERN.test(normalized)) {
    throw new Error(
      "a vulnerability class is 3 to 48 lowercase letters, digits or hyphens, such as sql-injection",
    )
  }
...
/**
 * A demonstrated proof is a failing mandatory gate: the candidate has the vulnerability, and
 * arbitration reads the evidence table, so the approval is refused without anyone having to
 * remember. A proof that did not demonstrate anything is recorded and blocks nothing.
 */
export function proofEvidence(
