import { parseCommand, UnsafeCommand } from "./commands.ts"
import { normalizeScope, scopesOverlap } from "./scopes.ts"
...
function parseTask(raw: unknown): Task {
  ...
  for (const scope of writeScopes) assertProjectRelative(scope)
  ...
  for (const command of verificationCommands) {
    try {
      ...
      if (error instanceof UnsafeCommand) throw new PlanRejected(error.message)
      throw error
    }
  }
...
/** A requirement nothing implements is a plan that cannot satisfy the request it came from. */
...
function assertDependenciesResolve(tasks: readonly Task[]): void {
  const keys = new Set(tasks.map((task) => task.key))
  for (const task of tasks) {
    for (const dependency of task.dependencies) {
      if (dependency === task.key) throw new PlanRejected(`task ${task.key} depends on itself`)
...
/**
 * Two tasks writing the same area must be ordered, or the second silently overwrites the first and
 * the candidate no longer matches either task's evidence.
 */
function assertScopesOrdered(tasks: readonly Task[]): void {
  const reachable = transitiveDependencies(tasks)

  for (const left of tasks) {
    for (const right of tasks) {
      if (left.key >= right.key) continue
      if (!scopesOverlap(left.writeScopes, right.writeScopes)) continue
      if (
        !reachable.get(left.key)?.has(right.key) &&
        !reachable.get(right.key)?.has(left.key)
      ) {
        throw new PlanRejected(
          `tasks ${left.key} and ${right.key} write overlapping scopes without an ordering between them`,
        )
      }
    }
  }
...
function transitiveDependencies(tasks: readonly Task[]): Map<string, Set<string>> {
  ...
  const resolve = (key: string, seen: Set<string>): Set<string> => {
    ...
    for (const dependency of direct.get(key) ?? []) {
      ...
      all.add(dependency)
      for (const nested of resolve(dependency, new Set([...seen, dependency]))) all.add(nested)
    }
    result.set(key, all)
    return all
  }

  for (const task of tasks) resolve(task.key, new Set([task.key]))
  return result
}
...
function exactKeys(raw: unknown, keys: readonly string[]): Record<string, unknown> {
  ...
  if (actual.length !== expected.length || actual.some((key, index) => key !== expected[index])) {
    throw new PlanRejected(
      `plan section must have exactly these keys: ${expected.join(", ")} (received: ${actual.join(", ") || "none"})`,
    )
  }
  return raw as Record<string, unknown>
}

function list(raw: unknown, field: string, allowEmpty: boolean): readonly unknown[] {
...
function strings(raw: unknown, field: string, allowEmpty = true): string[] {
  const items = list(raw, field, allowEmpty)
  return items.map((entry) => text(entry, `${field} entry`, MAX_TEXT))
}

function text(raw: unknown, field: string, maximum: number): string {
